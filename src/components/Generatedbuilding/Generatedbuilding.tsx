...
const Person = observer(({ offsetX, offsetY, name }: { name: string; offsetX: number; offsetY: number }) => {
  ...
  return (
    <div className={style.person} style={{ transform: `translate(${offsetX}px,${offsetY}px)` }}>
      {isSm ? (
        <Tooltip open={person === name} title={name}>
...
const Generatedbuilding: React.FC<GeneratedbuildingProps> = ({ className }) => {
  const {
    building: { config, people, receivePeople, dispose }
  } = useStore()
  const isSm = useRespondTo({ type: 'gte', breakpoint: 'sm' })
  useEffect(() => {
    if (config?.polygonName) receivePeople()
    return () => dispose()
  }, [config])
