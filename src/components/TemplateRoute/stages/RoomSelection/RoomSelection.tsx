...
const RoomSelection: React.FC<RoomSelectionProps> = ({ className, setContinue }) => {
  ...
  const saveRoom = useCallback(() => {
    ...
    setAbleToRoom(false)
    if (
      Object.keys(cs.rooms).reduce((_sum, roomKey) => _sum + cs.rooms[roomKey].zones.length, 0) ===
      _.flatten(cs.areas).filter(area => area.startsWith('b')).length
    ) {
      setContinue(true)
    }
  ...
  return (
    <>
      ...
      <MapTemplate style={{ gridTemplateAreas: cs.areasGridStyle }}>
        {_.flatten(cs.areas).map(area => (
          <div
            key={area}
            style={{
              backgroundColor: ~zones.indexOf(area)
                ? color
                : Object.values(cs.rooms).find(elem => elem.zones.includes(area))?.color
            }}
      ...
      {!ableToRoom ? (
        ...
        <>
          <Grid container direction='row' justify='center' alignItems='center' style={{ marginBottom: 'auto' }}>
            <input type='color' onChange={e => setColor(e.target.value)} style={{ marginRight: '15px' }} />
            <FormControlLabel
              control={<Checkbox checked={withDevices} onChange={toggleWithDevices} color='primary' />}
              label='С датчиками?'
            />
            {isSm && (
              <>
